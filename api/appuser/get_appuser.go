...
import (
	...
	appusermw "github.com/NpoolPlatform/appuser-gateway/pkg/middleware/appuser"
...
func (s *Server) GetUser(ctx context.Context, in *appuser.GetUserRequest) (*appuser.GetUserResponse, error) {
	...
	if _, err := uuid.Parse(in.GetUserID()); err != nil {
		...
		return &appuser.GetUserResponse{}, status.Error(npool.ErrParams, appuser.ErrMsgUserIDInvalid)
	}
...
func (s *Server) GetUserByAccount(ctx context.Context, in *appuser.GetUserByAccountRequest) (*appuser.GetUserByAccountResponse, error) {
	...
	if _, err := uuid.Parse(in.GetAppID()); err != nil {
		...
		return &appuser.GetUserByAccountResponse{}, status.Error(npool.ErrParams, appuser.ErrMsgAppIDInvalid)
	}

	if in.GetAccount() == "" {
		logger.Sugar().Error("Account empty")
		return &appuser.GetUserByAccountResponse{}, status.Error(npool.ErrParams, appuser.ErrMsgAccountEmpty)
	}
...
func (s *Server) GetAppUser(ctx context.Context, in *appuser.GetAppUserRequest) (*appuser.GetAppUserResponse, error) {
	...
	if _, err := uuid.Parse(in.GetUserID()); err != nil {
		logger.Sugar().Error("ID is invalid")
		return &appuser.GetAppUserResponse{}, status.Error(npool.ErrParams, appuser.ErrMsgUserIDInvalid)
	}

	if _, err := uuid.Parse(in.GetTargetAppID()); err != nil {
		logger.Sugar().Error("ID is invalid")
		return &appuser.GetAppUserResponse{}, status.Error(npool.ErrParams, appuser.ErrMsgAppIDInvalid)
	}
